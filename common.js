...
async function apiFetch(sheet, params) {
  ...
  if (params.action !== 'list' && !data.ok) throw new Error('操作未完成，請確認 Apps Script 已重新部署');
  return data;
}
...
function setStar(v) { currentStar = v; renderStars(); }
function renderStars() {
  document.querySelectorAll('#starInput span').forEach(s =>
    s.classList.toggle('on', parseInt(s.dataset.v) <= currentStar));
}
